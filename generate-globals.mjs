...
const outputFile = fs.existsSync(srcPath)
    ? path.resolve('src/globals.d.ts')
    : path.resolve('globals.d.ts')
...
for (const { importName } of modules) {
    ...
    for (const { name, generics } of types) {
        // Extract just the generic parameter names without constraints
        ...
        const genericParams = genericNames ? `<${genericNames}>` : ''
        fileContent += `  type ${name}${genericParams} = ${importName}.${name}${genericParams};\n`
    }

    // Interface handling remains the same
