...
export abstract class ConversationState<T = unknown> {
	...
	constructor(outputPort: OutputPort)
	constructor(props: T)
	constructor(outputPort: OutputPort, props: T)
	constructor(outputPortOrProps?: OutputPort | T, props?: T) {
	...
	getNextState(message = ''): Promise<Nullable<StateTransitionIntention>> {
		return Promise.resolve(null)
	}

	onEnter(): Promise<void> | void {
		logger.debug(`[onEnter] State: ${this.constructor.name} - Not implemented`)
	}

	onExit(): Promise<void> | void {
		logger.debug(`[onExit] State: ${this.constructor.name} - Not implemented`)
	}
